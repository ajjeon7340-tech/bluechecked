...
export const replyToMessage = async (messageId: string, replyText: string, isComplete: boolean, attachmentUrl?: string | null): Promise<void> => {
    ...
    if (replyText.trim() || attachmentUrl) {
        // Role depends on whether the current user is the creator (creator_id) or the original sender.
        // This ensures correct display in both incoming threads (fan → creator) and outgoing threads (admin welcome → user).
